...
connection = pymysql.connect(host='localhost',user='root',password='' , port=3307, db='resume')
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                name = resume_data.get('name', 'Guest')  # Fallback to 'Guest' if name is None or missing
                st.success(f"Hello {name}")
                
                st.subheader("**Your Basic info**")
               
                st.text('Name: '+ (resume_data.get('name', 'N/A')or 'N/A'))
                st.text('Email: ' + (resume_data.get('email', 'N/A')or 'N/A'))
                st.text('Contact: ' + (resume_data.get('mobile_number', 'N/A')or 'N/A'))
                st.text('Resume pages: '+ str(resume_data.get('no_of_pages', 'N/A')or 'N/A'))
                ...
                def insert_data(name, email, resume_score, timestamp, page_no, predicted_field, user_level, actual_skills, recommended_skills, recommended_courses):
                    insert_sql = f"""
